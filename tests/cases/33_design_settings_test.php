...
function clear_design_settings(): void
{
    ensure_test_db();
    \App\Core\Database::pdo()->exec("DELETE FROM settings WHERE `key` LIKE 'design_%' OR `key` IN ('color_primary', 'color_accent', 'font_family', 'font_heading', 'font_face_name', 'font_url', 'default_theme')");
    $ref = new \ReflectionClass(\App\Models\Setting::class);
